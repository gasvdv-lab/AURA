...
for(const id of ['feedback','workflow-help','step','run','trial','observe','remember','recall','believe','propose','evaluate','new-hypothesis','compile','test-model','live-model','relay-provider','relay-endpoint','relay-model','model-input','reset'])if(!page.includes(`id="${id}"`))throw new Error(`Ontbrekend interface-element: ${id}`);
