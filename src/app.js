...
const KEY='aura-memory-kernel-v0.6.0';let world,embodiment,sensorimotor,perception,memory;
const memoryEvents=()=>({emit:(type,payload,causeIds=[])=>world.emit(type,payload,causeIds)});
try{const saved=JSON.parse(localStorage.getItem(KEY));world=WorldKernel.restore(saved.world);embodiment=EmbodimentKernel.restore(world,saved.embodiment);sensorimotor=SensorimotorKernel.restore(embodiment,saved.sensorimotor);perception=PerceptionKernel.restore(world,embodiment,saved.perception);memory=MemoryKernel.restore(saved.memory,memoryEvents());}catch{world=createDefaultWorld();embodiment=new EmbodimentKernel(world);embodiment.create({id:'aura-body',origin:vec3(0,2,0)});world.createBody({id:'perception-fixture',materialId:'inert-standard',mass:2,radius:.35,position:vec3(0,4.2,-3),static:true});sensorimotor=new SensorimotorKernel(embodiment);perception=new PerceptionKernel(world,embodiment);memory=new MemoryKernel(memoryEvents());}
const status=document.querySelector('#status'),log=document.querySelector('#log');
const save=()=>localStorage.setItem(KEY,JSON.stringify({world:world.snapshot(),embodiment:embodiment.snapshot(),sensorimotor:sensorimotor.snapshot(),perception:perception.snapshot(),memory:memory.snapshot()}));
function render(){const e=embodiment.embodiments.get('aura-body'),latest=perception.observations.at(-1)??null;status.textContent=`tick ${world.tick} · ${e?Object.keys(e.bodyIds).length:0} body parts · ${perception.observations.length} perceptions · ${memory.traces.size} traces · ${memory.memories.size} memories`;log.textContent=JSON.stringify({objectiveWorld:world.worldState(),embodiment:embodiment.snapshot(),sensorimotor:sensorimotor.snapshot(),perceivedState:latest,memoryState:memory.snapshot(),latestEvents:world.observatory.snapshot().slice(-20)},null,2);}
document.querySelector('#step').onclick=()=>{embodiment.step(1);memory.advance(world.tick);save();render();};
document.querySelector('#run').onclick=()=>{embodiment.step(60);memory.advance(world.tick);save();render();};
...
document.querySelector('#remember').onclick=()=>{const latest=perception.observations.at(-1);if(latest)memory.record(latest);save();render();};
document.querySelector('#recall').onclick=()=>{memory.retrieve({track:'track-1',currentTick:world.tick});save();render();};
