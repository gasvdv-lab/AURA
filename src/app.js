import {createDefaultWorld} from './core/world/default-world.js';
import {WorldKernel} from './core/world/world-kernel.js';
import {EmbodimentKernel} from './embodiment/embodiment-kernel.js';
import {SensorimotorKernel} from './sensorimotor/sensorimotor-kernel.js';
import {vec3} from './core/world/vector3.js';
if('serviceWorker' in navigator)navigator.serviceWorker.register('./sw.js').catch(()=>{});
const KEY='aura-sensorimotor-kernel-v0.4.0';let world,embodiment,sensorimotor;
try{const saved=JSON.parse(localStorage.getItem(KEY));world=WorldKernel.restore(saved.world);embodiment=EmbodimentKernel.restore(world,saved.embodiment);sensorimotor=SensorimotorKernel.restore(embodiment,saved.sensorimotor);}catch{world=createDefaultWorld();embodiment=new EmbodimentKernel(world);embodiment.create({id:'aura-body',origin:vec3(0,2,0)});sensorimotor=new SensorimotorKernel(embodiment);}
const status=document.querySelector('#status'),log=document.querySelector('#log');
const save=()=>localStorage.setItem(KEY,JSON.stringify({world:world.snapshot(),embodiment:embodiment.snapshot(),sensorimotor:sensorimotor.snapshot()}));
function render(){const e=embodiment.embodiments.get('aura-body');status.textContent=`tick ${world.tick} · ${e?Object.keys(e.bodyIds).length:0} body parts · ${sensorimotor.trials.length} sensorimotor trials · ${world.observatory.records.length} causal events`;log.textContent=JSON.stringify({world:world.worldState(),embodiment:embodiment.snapshot(),sensorimotor:sensorimotor.snapshot(),rawSignals:sensorimotor.sample('aura-body'),latestEvents:world.observatory.snapshot().slice(-20)},null,2);}
document.querySelector('#step').onclick=()=>{embodiment.step(1);save();render();};
document.querySelector('#run').onclick=()=>{embodiment.step(60);save();render();};
document.querySelector('#trial').onclick=()=>{const e=embodiment.embodiments.get('aura-body');sensorimotor.runTrial({embodimentId:'aura-body',actuatorId:e.actuators[0].id,command:.25,steps:4});save();render();};
document.querySelector('#reset').onclick=()=>{localStorage.removeItem(KEY);location.reload();};render();
